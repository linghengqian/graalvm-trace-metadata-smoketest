...
public final class MultiThreadedTest {
    ...
    List<BiConsumer<LoadingCache<Int, Int>, Int>> operations = List.of(
            ...
            (cache, key) -> {
                cache.getAllPresent(List.of(key));
            },
            (cache, key) -> {
                cache.put(key, key);
            },
            (cache, key) -> {
                cache.putAll(Map.of(key, key));
            },
            (cache, key) -> {
            ...
            (cache, key) -> {
                checkState(cache.estimatedSize() >= 0);
            },
            (cache, key) -> {
                cache.stats();
            },
            ...
            (cache, key) -> {
                cache.asMap().putAll(Map.of(key, key));
            },
            (cache, key) -> {
            ...
            (cache, key) -> {
                cache.asMap().entrySet().toArray(new Map.Entry[cache.asMap().size()]);
            },
            (cache, key) -> {
                cache.hashCode();
            },
