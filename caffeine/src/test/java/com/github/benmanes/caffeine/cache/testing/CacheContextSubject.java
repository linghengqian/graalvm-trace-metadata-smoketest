...
import static com.github.benmanes.caffeine.cache.testing.CacheContextSubject.ListenerSubject.*;
...
public final class CacheContextSubject extends Subject {
    ...
    public void hasWeightedSize(long expectedSize) {
        ...
        actual.cache().policy().eviction().ifPresentOrElse(policy -> check("weightedSize()").about(optionalLongs()).that(policy.weightedSize()).hasValue(expectedSize), () -> {
            long weight = actual.cache().asMap().entrySet().stream().mapToLong(entry -> actual.weigher().weigh(entry.getKey(), entry.getValue())).sum();
    ...
    public StatsSubject stats() {
        return actual.isRecordingStats() ? check("stats").about(STATS_FACTORY).that(actual) : ignoreCheck().about(STATS_FACTORY).that(actual);
    }
    ...
    private void checkBasic(CacheEntry<Int, Int> entry) {
        check("entry").about(cache()).that(actual.cache()).containsEntry(entry.getKey(), entry.getValue());
        check("snapshotAt").that(entry.snapshotAt()).isEqualTo((actual.expires() || actual.refreshes()) ? actual.ticker().read() : 0L);
        try {
    ...
    private void checkWeight(CacheEntry<Int, Int> entry) {
        @SuppressWarnings("unchecked") var cache = (Cache<Int, Int>) actual.cache();
    ...
    private void checkExpiresAt(CacheEntry<Int, Int> entry) {
        ...
        cache.policy().expireAfterAccess().ifPresent(policy -> {
            var duration = policy.getExpiresAfter().minus(policy.ageOf(entry.getKey()).orElseThrow());
            check("expiresAccessAt").that(NANOSECONDS.toSeconds(expiresAt)).isEqualTo(NANOSECONDS.toSeconds(now + duration.toNanos()));
            check("expiresAccessAfter").that(entry.expiresAfter().toSeconds()).isEqualTo(duration.toSeconds());
        });
        cache.policy().expireAfterWrite().ifPresent(policy -> {
            var duration = policy.getExpiresAfter().minus(policy.ageOf(entry.getKey()).orElseThrow());
            check("expiresWriteAt").that(NANOSECONDS.toSeconds(expiresAt)).isEqualTo(NANOSECONDS.toSeconds(now + duration.toNanos()));
            check("expiresWriteAfter").that(entry.expiresAfter().toSeconds()).isEqualTo(duration.toSeconds());
        });
        cache.policy().expireVariably().ifPresent(policy -> {
            ...
            check("expiresVariablyAfter").that(entry.expiresAfter()).isEqualTo(policy.getExpiresAfter(entry.getKey()).orElseThrow());
        });
    }
    ...
    private void checkRefreshableAt(CacheEntry<Int, Int> entry) {
        ...
        cache.policy().refreshAfterWrite().ifPresent(policy -> {
            var duration = policy.getRefreshesAfter().minus(policy.ageOf(entry.getKey()).orElseThrow());
            check("refreshableAt").that(NANOSECONDS.toSeconds(refreshableAt)).isEqualTo(NANOSECONDS.toSeconds(now + duration.toNanos()));
            check("refreshableAfter").that(entry.refreshableAfter().toSeconds()).isEqualTo(duration.toSeconds());
        });
    }
    ...
    public static final class StatsSubject extends Subject {
        ...
        private StatsSubject(FailureMetadata metadata, CacheContext context) {
            ...
            this.actual = context;
            this.isDirect = !context.isRecordingStats() || (context.executorType() == CacheExecutor.DIRECT);
        }
        ...
        @CanIgnoreReturnValue
        private StatsSubject awaitStatistic(String label, ToLongFunction<CacheStats> supplier, long expectedValue) {
            if (isDirect) {
                checkStatistic(label, supplier, expectedValue);
            } else if (supplier.applyAsLong(actual.stats()) != expectedValue) {
                await().pollInSameThread().untilAsserted(() -> checkStatistic(label, supplier, expectedValue));
            }
            return this;
        }

        private void checkStatistic(String label, ToLongFunction<CacheStats> supplier, long expectedValue) {
            var stats = actual.stats();
            check(label).withMessage("%s", stats).that(supplier.applyAsLong(stats)).isEqualTo(expectedValue);
        }
    }

    public static final class ListenerSubject extends Subject {
        static final Factory<ListenerSubject, CacheContext> REMOVAL_LISTENER_FACTORY = factoryOf(RemovalListenerType.REMOVAL_LISTENER);
        static final Factory<ListenerSubject, CacheContext> EVICTION_LISTENER_FACTORY = factoryOf(RemovalListenerType.EVICTION_LISTENER);
        static final Factory<ListenerSubject, CacheContext> LISTENERS_FACTORY = factoryOf(RemovalListenerType.values());
        private final Map<RemovalListenerType, RemovalListener<Int, Int>> actual;
        private final boolean isDirect;

        private ListenerSubject(FailureMetadata metadata, CacheContext context, Map<RemovalListenerType, RemovalListener<Int, Int>> subject) {
            super(metadata, subject);
            this.actual = subject;
            this.isDirect = (context.executorType() == CacheExecutor.DIRECT);
        }

        private static Factory<ListenerSubject, CacheContext> factoryOf(RemovalListenerType... removalListenerTypes) {
            return (metadata, context) -> {
                var subject = Stream.of(removalListenerTypes).filter(listener -> listener.isConsumingListener(context)).collect(toImmutableMap(identity(), listener -> listener.instance(context)));
        ...
        private void awaitUntil(BiConsumer<RemovalListenerType, ConsumingRemovalListener<Int, Int>> consumer) {
        ...
        public final class WithCause {
            ...
            public Exclusive contains(Int key, Int value) {
                awaitUntil((type, listener) -> check(type).withMessage("%s", cause).that(listener.removed()).contains(new RemovalNotification<>(key, value, cause)));
            ...
            public Exclusive contains(Entry<?, ?>... entries) {
                awaitUntil((type, listener) -> {
                    var notifications = Stream.of(entries).map(entry -> new RemovalNotification<>(entry.getKey(), entry.getValue(), cause)).collect(toImmutableSet());
                    check(type).withMessage("%s", cause).that(listener.removed()).containsAtLeastElementsIn(notifications);
                });
            ...
            public final class Exclusive {
                ...
                public void exclusively() {
                    awaitUntil((type, listener) -> {
                        var causes = listener.removed().stream().map(RemovalNotification::getCause).collect(toImmutableMultiset());
                        check(type).that(causes).isEqualTo(ImmutableMultiset.builder().addCopies(cause, Math.toIntExact(expectedSize)).build());
                    });
                }
            }
        }
    ...
    private enum RemovalListenerType {
        REMOVAL_LISTENER(CacheContext::removalListenerType, CacheContext::removalListener), EVICTION_LISTENER(CacheContext::evictionListenerType, CacheContext::evictionListener);
        ...
        RemovalListenerType(Function<CacheContext, Listener> listener, Function<CacheContext, RemovalListener<Int, Int>> instance) {
